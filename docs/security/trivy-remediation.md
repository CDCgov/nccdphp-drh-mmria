...
- **Usage/reachability:** Repo-static verification (Tier 1, run 2026-07-24): `find <repo-root> -name 'package.json' -not -path '*/node_modules/*'` returns **zero results** — no npm dependency manifest exists anywhere in this repository. The only `npm install` invocation in the S2I build path is in `.s2i/bin/assemble` line 209 (`npm install ${DOTNET_NPM_TOOLS}`), gated by `if [ -n "${DOTNET_NPM_TOOLS}" ]`. A repo-wide search (`grep -rn "DOTNET_NPM_TOOLS"`) shows this variable is referenced only at `.s2i/bin/assemble:192,209` and is **never set** in `.s2i/environment` or any other build/CI configuration file in this repo — the npm-install branch is dead code for this build. The `dotnet-host` RPM does not ship any npm packages.
- **Exploit preconditions:** The supply-chain attack required installation of the compromised `debug@4.4.2` npm package. This RPM package is not npm; the attack vector is entirely absent, and the only code path that could install an npm package in this build is unreachable because `DOTNET_NPM_TOOLS` is unset.
- **Compensating controls:** No npm packages are installed in the MMRIA S2I image. The image does not include Node.js or npm tooling, and the conditional npm-install step in `.s2i/bin/assemble` never executes with this repo's configuration.
...
- **Follow-up:** None required. This is a Trivy cross-ecosystem CVE misattribution, confirmed by static analysis showing the npm-install code path is unreachable in this repo's build configuration.
...
- **Usage/reachability:** Repo-static verification (Tier 1, run 2026-07-24): no `package.json` exists anywhere in this repository (`find <repo-root> -name 'package.json' -not -path '*/node_modules/*'` → zero results), so no `ws` npm dependency is declared anywhere. The only path that could ever install an npm package during the S2I build is `.s2i/bin/assemble:209` (`npm install ${DOTNET_NPM_TOOLS}`), gated on `DOTNET_NPM_TOOLS` being non-empty; that variable is never set in `.s2i/environment` or anywhere else in the repo, so the branch never executes. No Node.js runtime or npm registry interaction occurs in the `dotnet-host` installation or execution path.
- **Exploit preconditions:** The vulnerability requires an attacker to send WebSocket messages to a Node.js server using the `ws` package, causing unbounded buffer allocation. No `ws` npm package is installed in this image and no Node.js WebSocket server runs in the S2I builder; the code path that could install it is unreachable given this repo's build configuration.
- **Compensating controls:** No Node.js or npm packages are installed in the MMRIA S2I image. The image uses the .NET SDK only, and the conditional npm-install step in `.s2i/bin/assemble` never executes with this repo's configuration.
...
- **Usage/reachability:** Repo-static verification (Tier 1, run 2026-07-24): no `package.json` exists anywhere in this repository (`find <repo-root> -name 'package.json' -not -path '*/node_modules/*'` → zero results), so `node-tar` is not declared as a dependency anywhere. The only conditional path that could install any npm package during the S2I build is `.s2i/bin/assemble:209`, gated on `DOTNET_NPM_TOOLS`; that variable is never set in `.s2i/environment` or elsewhere in the repo, so this path never executes and no npm package — including `node-tar` — is ever installed. The `tar` binary present in the image is GNU tar (C implementation, RPM-installed). The vulnerability class (unbounded decompression in JavaScript path parsing) does not apply to GNU tar.
- **Exploit preconditions:** Requires an attacker to supply a malicious tar archive to a Node.js application using `node-tar < 7.5.19`. No Node.js runtime or `node-tar` npm package exists in the MMRIA S2I image, and the only code path that could install one is unreachable in this repo's build configuration.
...
- **Follow-up:** None required. This is a Trivy cross-ecosystem CVE misattribution (npm CVE vs. OS RPM package), confirmed by static analysis showing the npm-install code path is unreachable in this repo's build configuration.
...
- **Usage/reachability:** Repo-static verification (Tier 1, run 2026-07-24): no `package.json` exists anywhere in this repository, confirming `node-tar` is not a declared dependency. The only conditional path that could install any npm package (`.s2i/bin/assemble:209`, gated on `DOTNET_NPM_TOOLS`) never executes because that variable is unset in `.s2i/environment` and nowhere else in the repo. The `tar` binary in the MMRIA S2I image is GNU tar (RPM-installed C binary). The specific `tar.replace` function vulnerable in CVE-2026-59874 is a JavaScript API that does not exist in GNU tar.
- **Exploit preconditions:** Requires a Node.js application calling `tar.replace` from the `node-tar` npm package with a specially crafted archive. No Node.js runtime or `node-tar` package is present in this image, and the only path that could install one is unreachable in this repo's build configuration.
...
CVE-2025-59144 documents the September 2025 takeover of the `debug` npm account and publication of malware-laced `debug@4.4.2`. The affected artifact is a JavaScript npm package. The Red Hat `dotnet-host` RPM is the .NET runtime host binary — it contains no JavaScript code and does not bundle or execute the `debug` npm package. Trivy incorrectly matched this CVE to `dotnet-host` due to a vulnerability-database cross-ecosystem mapping error. The `mmria-s2i` image contains no npm packages, no Node.js runtime, and no JavaScript dependency manager. Repo-static verification (2026-07-24): a repository-wide search finds zero `package.json` files, and the sole `npm install` invocation in the S2I build (`.s2i/bin/assemble:209`) is gated on `DOTNET_NPM_TOOLS`, a variable that is never set anywhere in this repository — the npm-install branch is unreachable dead code for this build.
...
CVE-2026-48779 affects the `ws` npm package (Node.js WebSocket library) across multiple version ranges below the patched releases. The vulnerability requires an attacker to send specially crafted WebSocket messages to a Node.js `ws`-based server. The Red Hat `dotnet-host` RPM provides the .NET runtime host; it ships no npm packages and is not a Node.js WebSocket server. Trivy misattributed this npm CVE to the `dotnet-host` RPM. The MMRIA S2I image contains no Node.js runtime and no npm packages; the `ws` library is entirely absent. Repo-static verification (2026-07-24): no `package.json` exists in the repository, and the only conditional npm-install path in `.s2i/bin/assemble` (gated on `DOTNET_NPM_TOOLS`) never fires because that variable is unset in `.s2i/environment` and everywhere else in the repo.
