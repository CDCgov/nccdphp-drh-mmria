...
public sealed class BackupHotProcessor : ReceiveActor
{
        ...
        string backupDbPassword)
    {
        ...
        replicateStruct.source.headers.Authorization = "Basic " + Base64Encode($"{workItem.SourceUserName}:{workItem.SourcePassword}");
        replicateStruct.target.url = CombineUrl(backupDbUrl, workItem.TargetDatabaseName);
        replicateStruct.target.headers.Authorization = "Basic " + Base64Encode($"{backupDbUser}:{backupDbPassword}");
