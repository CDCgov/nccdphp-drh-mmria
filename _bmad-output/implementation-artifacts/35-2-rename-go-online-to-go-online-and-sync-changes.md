...
- `node docs/ai/local/epic-35-offline-ux/verify-35-2-go-online-rename.js` — RED before implementation (7 assertions failed, all expected), GREEN after implementation (11/11 assertions passed). Re-verified GREEN (11/11) after the follow-up two-line button styling change.
...
- **Side finding (pre-existing, not fixed — out of scope):** `set_go_online_button_state` and the connectivity handler in `offline-network-monitor.js` both look up `goOnlineButton.querySelector('.button-text')`, but neither button's markup in `app.mmria.js` contains an element with class `button-text` — only a bare text node after the `<img>` tag. This means the busy-state (`'Going Online...'`) and connectivity-disabled text updates are currently inert no-ops (the `if (buttonText)` guard silently skips). The idle-state label change made in this story is still correct and fully visible because it also updates the raw text node directly in `app.mmria.js`'s template — that's what users actually see. Flagging this mismatch for a future story since fixing it would mean adding click-handler/DOM-structure changes, which are out of this story's string-only scope.
- Verification approach: created a Node-based static-content verification harness (`docs/ai/local/epic-35-offline-ux/verify-35-2-go-online-rename.js`) since this codebase has no JS test runner for `wwwroot/` (no build step, no package.json). The harness asserts exact string content at each named site — this is a full substitute for the AC's literal text requirements, but does not exercise a live browser. Recommend a final manual smoke test (go offline, observe both buttons/modal) before merging, per the story's last task.
...
- `source-code/mmria/mmria-server/wwwroot/scripts/editor/page_renderer/app.mmria.js` — renamed both `go-online-btn` labels; restyled the "Offline Case List" button to two centered lines
- `source-code/mmria/mmria-server/wwwroot/scripts/offline/offline-transition-manager.js` — renamed idle-state text in `set_go_online_button_state`
- `source-code/mmria/mmria-server/wwwroot/scripts/offline/offline-network-monitor.js` — renamed both connectivity-branch button text assignments
...
- `docs/ai/local/epic-35-offline-ux/verify-35-2-go-online-rename.js` — new static-content verification harness (11 assertions); updated button-#1 assertion for two-line markup
