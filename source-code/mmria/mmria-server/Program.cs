        ...
        try
        {
            ...
            (
                new Action(async () =>
                {
                    ...
                    // Seed validation rules after database setup
                    try
                    {
                        Log.Information("Starting validation rules seeding");
                        using var seedingScope = app.Services.CreateScope();
                        var caseValidationManager = seedingScope.ServiceProvider.GetRequiredService<mmria.common.SharedLibraries.CaseValidation.Manager.CaseValidationManager>();
                        var metadataVersionManager = seedingScope.ServiceProvider.GetRequiredService<mmria.common.SharedLibraries.MetadataVersion.Manager.MetadataVersionManager>();

                        if (multiTenantJurisdictions.Length > 0)
                        {
                            // Multi-tenant: seed once per tenant using its own prefix and db config
                            for (int i = 0; i < multiTenantJurisdictions.Length; i++)
                            {
                                var tenantPrefix = multiTenantJurisdictions[i].Trim();
                                var tenantMetadataVersion = overridableConfigSets[i]?.GetString("metadata_version", tenantPrefix);
                                Log.Information($"Validation seeding tenant '{tenantPrefix}': metadata_version='{tenantMetadataVersion}'");
                                if (string.IsNullOrEmpty(tenantMetadataVersion)) { Log.Warning($"Skipping tenant '{tenantPrefix}': no metadata_version"); continue; }

                                var tenantDbConfig = overridableConfigSets[i]?.GetDBConfig(tenantPrefix);
                                if (tenantDbConfig == null) { Log.Warning($"Skipping tenant '{tenantPrefix}': GetDBConfig returned null"); continue; }
                                Log.Information($"Validation seeding tenant '{tenantPrefix}': db url='{tenantDbConfig.url}'");

                                var tenantMetadata = await metadataVersionManager.GetAppMetadataAsync(tenantMetadataVersion, tenantDbConfig);
                                if (tenantMetadata == null) { Log.Warning($"Skipping tenant '{tenantPrefix}': GetAppMetadataAsync returned null for version '{tenantMetadataVersion}'"); continue; }

                                var tenantDoc = await caseValidationManager.GetOrCreateRuleDocumentAsync(
                                    tenantMetadataVersion,
                                    tenantMetadata,
                                    tenantDbConfig,
                                    "system");
                                await caseValidationManager.SaveRuleDocumentAsync(
                                    tenantMetadataVersion,
                                    tenantDoc,
                                    tenantDbConfig,
                                    "system");
                                Log.Information($"Seeded validation rules for tenant '{tenantPrefix}'");
                            }
                        }
                        else
                        {
                            // Single-tenant: use config_id as prefix
                            var metadataVersion = overridableConfigSets[0]?.GetString("metadata_version", config_id);
                            Log.Information($"Validation seeding single-tenant: config_id='{config_id}' metadata_version='{metadataVersion}'");
                            if (!string.IsNullOrEmpty(metadataVersion))
                            {
                                var dbConfig = overridableConfigSets[0]?.GetDBConfig(config_id);
                                if (dbConfig == null) { Log.Warning($"Skipping single-tenant: GetDBConfig('{config_id}') returned null"); }
                                else
                                {
                                    Log.Information($"Validation seeding single-tenant: db url='{dbConfig.url}'");
                                    var metadata = await metadataVersionManager.GetAppMetadataAsync(metadataVersion, dbConfig);
                                    if (metadata == null) { Log.Warning($"Skipping single-tenant: GetAppMetadataAsync returned null for version '{metadataVersion}'"); }
                                    else
                                    {
                                        var doc = await caseValidationManager.GetOrCreateRuleDocumentAsync(
                                            metadataVersion,
                                            metadata,
                                            dbConfig,
                                            "system");
                                        await caseValidationManager.SaveRuleDocumentAsync(
                                            ...
                                            dbConfig,
                                            "system");
                                        Log.Information("Seeded validation rules for single tenant mode");
                                    }
                                }
                            }
                        }
                        Log.Information("Completed validation rules seeding");
                    }
                    catch (Exception ex)
                    {
                        Log.Error($"Failed to seed validation rules during startup\n{ex}");
                    }
                }));
