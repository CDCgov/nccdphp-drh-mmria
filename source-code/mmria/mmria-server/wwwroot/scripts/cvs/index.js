...
async function run_cvs_report_polling(header, el, spinner, report_output_element, reset_log)
{
    ...
    try {
        for (let attempt = 1; attempt <= CVS_MAX_ATTEMPTS; attempt++) {
            ...
            el.innerHTML = "PDF cannot be generated.<br/><span style='color:#FF0000;'>External Community Vital Signs Server is unavailable.</span> Please try again later.";
            spinner.innerHTML = render_close_button_html();
            report_log.push(`CVS response Status Code: ${response.status || response.statusCode || "unknown"} @ ${new Date()} ${response.detail || ""}`);
            render_report_log(report_output_element);
            post_cvs_status("failed");
            window.setTimeout(() => { const close_button = document.getElementById("close_button"); close_button.focus(); }, 0);
            return;
...
async function get_cvs_api_dashboard_info(lat, lon, year, id)
{
    ...
    try {
        ...
        if (window.$mmria && typeof window.$mmria.add_request_verification_header === "function") {
            headers = window.$mmria.add_request_verification_header(headers);
        }
        ...
        if (!response.ok) {
            ...
            return {
                file_status: is_transient_status(response.status) ? "unavailable" : "error",
                status: response.status,
                external_status_code: response.status,
                external_reason_phrase: response.statusText,
                external_error_message: detail,
                detail: detail
            };
        }
...
{
    var base_url = `${location.protocol}//${location.host}/api/cvsAPI/${encodeURIComponent(p_id)}`;
