    ...
    for (let i = 0; i < g_file_stat_list.length; i++) 
    {
        ...
        if (typeof item !== "undefined") 
        {
            if (item.name.toLowerCase().endsWith(".mor")) 
            {
                ...
                const normalized_host_prefix = (host_prefix || "").toLowerCase();
                const is_current_host_additional_tenant =
                    g_vitals_import_additional_tenant_set.has(normalized_host_prefix) &&
                    mor_file_name_suffix === normalized_host_prefix;
                const additional_tenant_mor_regex = is_current_host_additional_tenant
                    ? build_additional_tenant_mor_regex([normalized_host_prefix])
                    : null;

				if(is_current_host_additional_tenant && additional_tenant_mor_regex){
                ...
                if (!is_current_host_additional_tenant && state_name.toUpperCase() != host_prefix) 
                {
