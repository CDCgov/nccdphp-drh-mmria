function render0()
{
    return `
        ...
        <p>
            The Overdose report in MMRIA grew out of the Rapid Maternal Overdose Review initiative. 
            This initiative ensures the MMRC scope is inclusive of full abstraction and review of all overdose deaths during and within one year of the end of pregnancy; 
            the MMRC is multidisciplinary and representative of maternal mental health, substance use disorder prevention, and addiction medicine; 
            and the team determines contributing factors and recommendations, regardless of whether the death is determined to be pregnancy-related.
        </p>
        <p>
            <span class="font-weight-bold">This report only includes deaths where the Means of Fatal Injury was “Poisoning/Overdose” in the Manner of Death section of the MMRIA Committee Decisions Form.</span> 
            The committee should be documenting means of fatal injury for all pregnancy-associated deaths, but if the committee is not consistently doing this the number of pregnancy-associated overdose deaths reviewed could be underestimated in the report.
        </p>
        <p>
            The Overdose Report can be used to look at broad categories of overdose deaths within MMRIA but should not replace more specific analysis. 
            For example, the Overdose Report is only able to show race/ethnicity as non-Hispanic Black, non-Hispanic White, Hispanic, and Other while an individual jurisdiction can look at other race/ethnicity groupings after downloading the data. 
        ...
        <table class="table table-hover table-hover-light nein-scroll">
            <thead>
                <tr class="header-level-2">
                    ...
                    <th width="250"><strong>Report Page</strong></th>
...
function render_table(p_metadata, p_data, p_totals, p_total)
{
    return `
        ...
        <div style="border-color: #d5d5d5 !important;" class="d-flex border rounded border-light text-left p-2 mt-3 mb-4 col-md-6"><span class="font-weight-bold">Number of deaths with missing (blank) values:</span><span class="ml-auto">${p_totals.get(p_metadata.blank_field_id)}</span></div>
    ...
    (
        `var ${p_chart_name} = c3.generate({
            ...
            data: {
                ...
                labels: true,
                colors: {
                    ${p_metadata.indicator_id}: '${BAR_CHART_COLOR}' // Set the bar color to purple
                }
            },
            padding: {
                  //left: 375
