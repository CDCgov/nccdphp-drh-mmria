...
function render_queue_result(q)
{
    ...
    if(q.items.length > 0)
    {
        ...
        for(const i in q.items)
        {
            ...
            {
                download_button = `<button class='secondary-button icon-button' onclick="window.open('stevePRAMS/GetFileResult?FileName=${q.items[i].fileName}', '_new')"><img src="/img/download.svg" alt="Download"></button>
