...
function page_render_create_textarea(p_result, p_metadata, p_data, p_metadata_path, p_object_path, p_dictionary_path)
{
	...
	{
		p_result.push(`<p class="mb-3" style="line-height: normal"><strong id="case-narrative-heading" style="font-size:1.1em">Case Narrative Template Guidelines</strong><br>• You may use this template as a guide, deleting any portions that are not applicable.<br>• Alternatively, you may copy the reviewer's notes sections below into the final case narrative field or into an external document. You may also use your own template.<br>• Ensure any narrative you want to copy and paste into the final case narrative field is in plain text without formatting (ctrl+shift+v).<br><br><strong style="font-size:1.1em">Remember to:</strong><br>• Focus on the most relative information to the cause of death (see Cause of Death Modules)<br>• Humanize the story using a story-telling approach<br>• Use inclusive and non-stigmatizing language<br>• Spell out acronyms or explain in plain text clinical terminology<br>• Incorporate interview(s) and CVS throughout (as applicable)</p>`);
		p_result.push(`<textarea class='form-control' ${disabled_html} id='case_narrative_editor' aria-labelledby='case-narrative-heading' name='`);
	}
	else
	{
