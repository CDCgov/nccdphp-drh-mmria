...
public sealed class clear_case_statusController : Controller
{
        ...
        mmria.server.model.casestatus.CaseStatusRequest Model)
    {
        ...
        try
        {
            ...
            var caseStatusRows = await _caseManager.FindCaseStatusRecordsAsync(Model.RecordId, effectiveDbConfig);
            foreach (var item in caseStatusRows)
            {
                try
                {
        ...
        mmria.server.model.casestatus.CaseStatusDetail Model)
    {
        ...
        try
        {
            ...
            var clearResult = await _caseManager.ClearCaseStatusAsync(model._id, userName, effectiveDbConfig);
            model.LastUpdatedBy = clearResult.LastUpdatedBy;
            model.DateLastUpdated = clearResult.DateLastUpdated;
            model.CaseStatusDisplay = clearResult.StatusText;
